...
export default function EncryptionStatusCard() {
  ...
  const isNotEnabled = !setupCompleted;
        ...
        <div className="flex items-center gap-3 min-w-0">
          <div
            className={`w-8 h-8 md:w-10 md:h-10 rounded-xl flex items-center justify-center shrink-0 ${
              !isNotEnabled && isUnlocked ? "bg-emerald-500/10" : !isNotEnabled && !isUnlocked ? "bg-amber-500/10" : "bg-[var(--border)]"
            }`}
          >
            {isNotEnabled ? (
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--muted)] opacity-40">
          ...
          <div className="min-w-0">
            <p className="text-sm font-bold text-[var(--foreground)] truncate">
              {isNotEnabled ? "Encryption not enabled" : isUnlocked ? "Encryption active" : "Encryption locked"}
            </p>
            <p className="text-[10px] md:text-xs text-[var(--muted)] truncate">
              {isNotEnabled ? "Set up encryption to protect your data" : isUnlocked ? "Keys cached in memory" : "Unlock with your passphrase"}
            </p>
          </div>
        </div>
